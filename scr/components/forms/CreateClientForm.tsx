    ...
    try {
      await apiFetch("/clients", {
        ...
        body: JSON.stringify({
          ...
          contactName: vals.name,
          contactTel: vals.telephone,
          contactEmail: vals.email,
  ...
  return (
    <form onSubmit={clientForm.handleSubmit(onSubmit)} className="space-y-3 mt-2">
      ...
      <div>
        ...
        <Input {...clientForm.register("name")} placeholder="Contact person" />
      </div>
      <div>
        <label className="text-xs font-medium">Telephone</label>
        <Input {...clientForm.register("telephone")} placeholder="+1 555 123 4567" />
      </div>
      <div>
        <label className="text-xs font-medium">Email</label>
        <Input type="email" {...clientForm.register("email")} placeholder="email@example.com" />
      </div>
      <div>
